...
| [`/llm-security`](commands/llm-security.md) | AI/LLM agent security (OWASP GenAI/LLM Top 10 2026): prompt injection (direct + indirect/RAG), jailbreaks, hidden-context & secret leakage, excessive agency / tool abuse, RAG exfiltration, improper output handling, cost-DoS, MCP attack surface (tool poisoning, rug pull, shadowing). Ships a red-team probe set. | Monthly (if you ship an AI agent) |
...
| [`/rls-audit`](commands/rls-audit.md) | Supabase RLS / Postgres authorization: unprotected tables, permissive policies, self-elevation vectors, `SECURITY DEFINER` (incl. tenant-from-payload), service-role leakage, policy performance (missing indexes on policy columns). | Monthly |
...
| [`/email-deliverability`](commands/email-deliverability.md) | Inbox-readiness: Gmail/Yahoo/Microsoft bulk-sender thresholds, SPF/DKIM/DMARC auth & alignment, policy enforcement + report analysis, ESP/IP reputation & blocklists, spam-score & content, transactional-vs-marketing separation, list hygiene, RFC 8058 one-click unsubscribe, warmup, bounce/complaint monitoring. | Quarterly |
